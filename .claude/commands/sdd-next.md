...
FEAT-007 -- Event Bus Core
  Active worktree: feat-007-eventbus-core
  1. TASK-003 -- DLQ Handler           [high / M]
     Depends-on: TASK-001 [done], TASK-002 [done]
     -> /sdd-start TASK-003  (run inside existing worktree)

FEAT-008 -- Hook System
  No worktree -- create one:
     git worktree add -b feat-008-hook-system .claude/worktrees/feat-008-hook-system HEAD
     cd .claude/worktrees/feat-008-hook-system
  2. TASK-010 -- HookManager base   [high / M]
     Depends-on: none
     -> /sdd-start TASK-010

FEAT-009 -- Ingress Models
  Parallel tasks (can run in separate worktrees):
  3. TASK-042 -- CloudEvents Model      [medium / S]
     git worktree add -b task-042-cloudevents .claude/worktrees/task-042-cloudevents HEAD
     -> cd .claude/worktrees/task-042-cloudevents && /sdd-start TASK-042
  4. TASK-043 -- AMQP Model        [medium / S]
     git worktree add -b task-043-amqp .claude/worktrees/task-043-amqp HEAD
     -> cd .claude/worktrees/task-043-amqp && /sdd-start TASK-043
```
...
```
In progress:
  TASK-002 -- EventPublisher [in feat-007-eventbus-core]
  TASK-021 -- DLQ Retry Logic  [in task-021-dlq-retry]
```
