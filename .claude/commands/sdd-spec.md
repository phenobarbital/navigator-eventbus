...
Scaffold a new Feature Specification for navigator-eventbus using the SDD methodology.
...
- **NEVER re-ask a question that the brainstorm already answered.** Resolved
  answers must be carried forward verbatim, not re-opened. See Section 2 for the
  resolved-question convention and Section 3 for what you MAY ask.
...
If neither exists, proceed to Section 3.
...
|---|---|
| Problem Statement | Section 1 Motivation -- Problem Statement (verbatim, condensed only if needed) |
| Constraints & Requirements | Section 1 Goals + Section 5 Acceptance Criteria (every hard constraint becomes a checkable criterion) |
| Recommendation + Recommended Option body | Section 2 Architectural Design -- Overview |
| Feature Description -> User-Facing Behavior | Section 2 Overview |
| Feature Description -> Internal Behavior | Section 2 Component Diagram + Integration Points |
| Feature Description -> Edge Cases & Error Handling | Section 7 Known Risks / Gotchas |
| Capabilities (New + Modified) | Section 3 Module Breakdown (one module per capability as a starting point) |
| Impact & Integration table | Section 2 Integration Points |
| Code Context (entire section) | Section 6 Codebase Contract (re-verify every reference -- code may have shifted) |
| Libraries / Tools table | Section 7 External Dependencies |
| Parallelism Assessment | Worktree Strategy section |
| Open Questions (see 2b) | Section 8 Open Questions (with resolved/unresolved state preserved) |

Rejected options from the brainstorm are NOT carried into the spec body.
They may be referenced in one line inside Section 1 Non-Goals if the rejection
...
1. **Do NOT re-ask the user.** Never include a resolved brainstorm question
   in the clarifying-question batch in Section 3.
2. **Route the answer into the spec body where the decision actually
   applies** -- not just into Section 8. For example:
   - "Default backend when unset -> redis" -> state this in Section 2 Overview and
     add an acceptance criterion in Section 5. Do not leave it as an open question.
   - "Event schema defined in `src/navigator_eventbus/models.py`" -> add the config
     key to Section 6 Configuration References and mention the path in Section 7
     Patterns to Follow. Do not describe the design as "mingled" or any
     alternative that contradicts the resolved answer.
3. **Also echo the resolution in Section 8** as a resolved item so readers can
   audit the decision trail:
   ```
   - [x] <Question restated> -- *Resolved in brainstorm*: <answer verbatim>
   ```
   This keeps Section 8 honest about what was decided and by whom.
4. **If a resolved answer conflicts with your own instinct for the spec**,
   ...
   in Section 3 -- do not rewrite the answer.

**Rules for unresolved (`[ ]`) questions:**

- Carry them forward into Section 8 of the spec as `[ ]` items.
- They are fair game for Section 3 clarifying questions, but only if they
  genuinely block the spec (ones that can be decided during
  implementation should stay as `[ ]` in Section 8 and not be asked now).

#### 2c. Show the user the carry-forward summary before asking anything

Before the clarifying-question round in Section 3, print:
...
If K is zero, proceed directly to Section 4 without asking anything.
...
```
type='hotfix' requires base_branch='main' (got base_branch='<value>').
...
```
Cannot sync <BASE_BRANCH>: working tree has uncommitted changes.
   Stash or commit first, then re-run /sdd-spec.
```

If `--ff-only` fails, abort with:
```
Cannot fast-forward <BASE_BRANCH>. Reconcile manually
   (git pull --rebase or merge), then re-run /sdd-spec.
```

Carry `TYPE` and `BASE_BRANCH` forward into the spec's frontmatter at Section 5.

### 3. Ask Clarifying Questions (only what is genuinely missing)

After Section 2c, you may ask the user **only** for gaps the brainstorm/proposal did
...
- Ambiguities discovered during codebase research in Section 4 (e.g., two plausible
...
- Identify related existing components.
...
3. **Verify all imports**: confirm that `from navigator_eventbus.X import Y` resolves by
...
2. Create `sdd/specs/<feature-name>.spec.md` filled in with:
   - **Frontmatter** at the very top: set `type` and `base_branch` to the
     values resolved in Section 2d. Do NOT strip the frontmatter. The block must
...
3. When populating Section 8 Open Questions, apply the resolved/unresolved
   partition from Section 2b. Resolved items use `[x]` and the carried-forward
